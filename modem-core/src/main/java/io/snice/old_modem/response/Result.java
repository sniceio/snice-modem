...
public class Result {
    ...
    public static List<Result> initialize(final boolean verbose, final char S3, final char S4) {
        final List<Result> results = new ArrayList<>(Code.values().length);
        ...
        Arrays.stream(Code.values()).forEach(rc -> {
            ...
            results.add(new Result(rc, encoded));
        });
    ...
    private Result(final Code code, final byte[] encoded) {
        this.code = code;
